...
| -------------------- | ------------------------------------------ | -------------------------- |
| `oauth_sign_in_started` | User starts Google/GitHub OAuth from login | provider |
| `oauth_sign_in_completed` | OAuth callback completes and user lands on dashboard | userId |
| `sign_out_clicked` | User clicks Sign out | userId |
...
These events are the only custom events in this project. Do not add more without updating this list first.
